...
def hard_swish  (x:float)->float:return x*hard_sigmoid(x)
...
def elu         (x:float)->float:return x if x > 0 else 0.01*(np.exp(x)-1)
def celu        (x:float)->float:return x if x > 0 else 0.01*(np.exp(x)-1)
def tanh        (x:float)->float:return np.tanh(x)
def tanh_shrink (x:float)->float:return x-np.tanh(x)
def tanh_exp    (x:float)->float:return np.tanh(np.exp(x))
